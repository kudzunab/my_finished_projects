...
from src.datasource.model.model import GameBase, UsersInfo, GameStats
...
class GameRepository:
        ...
        try:
            with self.session_factory() as session:
                ...
                if my_status == "waiting_players":
                    my_status = int(Status.waiting_players.value)    #0
                current_status = Status(my_status)
                my_game_type = db_game.game_type
                if db_game.game_type == "with_player":
                    my_game_type = int(Status.turn_player_with_UUID.value) #1
        ...
        try:
            ...
            with (self.session_factory() as session):
                query = select(GameBase).where(
                    GameBase.status == int(Status.waiting_players.value),
    ...
    def get_finished_game(self, player_uuid):
        try:
            with (self.session_factory() as session):
                query = select(GameBase.UUID, GameBase.game_type, GameBase.status,
                               GameBase.created_at).where(
                    or_(
                        GameBase.player1_uuid == player_uuid,
                        GameBase.player2_uuid == player_uuid
                    ),
                    or_(GameBase.status==int(Status.nobody_wins.value),
                        GameBase.status==int(Status.win_player_with_UUID.value))
    ...
    def change_game_stats(self, delta_win_games: int, user_uuid: str):
        try:
            with self.session_factory() as session:
                with session.begin():
                    db_stats = session.query(GameStats).where(
                        GameStats.user_uuid == user_uuid).with_for_update().one_or_none()
                    if db_stats is None:
                        db_stats = GameStats(user_uuid=user_uuid, wins_count=0, games_count=0)
                        session.add(db_stats)

                    db_stats.wins_count += delta_win_games
                    db_stats.games_count += 1
                    db_stats.wins_rate = db_stats.wins_count/db_stats.games_count
            return True
    ...
    def get_sorted_stats(self, n=10):
        try:
            with self.session_factory() as session:
                query = select(UsersInfo.login, GameStats.wins_rate) \
                    .join(UsersInfo, UsersInfo.uuid == GameStats.user_uuid)\
                    .order_by(desc(GameStats.wins_rate)).limit(n)

                result = session.execute(query).all()
                res_list = []
                for line in result:
                    res_list.append({"login": line[0], "win ratio": line[1]})
                return res_list
